...
async function validateOpcionByNumero(id_aventura, numero_pagina_origen) {
  return (
    (
      await conn.query(
        "SELECT 1 FROM opcion WHERE id_aventura = $1 AND numero_pagina_origen = $2 LIMIT 1",
        [id_aventura, numero_pagina_origen]
      )
    ).rowCount !== 0
  );
}
...
async function updateOpcionByNumero(
  ...
  try {
    if (!id_aventura || !numero_pagina_origen)
      throw new Error(
        "id_aventura y numero_pagina_origen de opcion son requeridos"
      );

    if (
      (await validateOpcionByNumero(id_aventura, numero_pagina_origen)) ===
      false
    )
    ...
    if (descripcion)
      await conn.query(
        "UPDATE opcion SET descripcion= $3 WHERE id_aventura = $1 AND numero_pagina_origen = $2",
        [id_aventura, numero_pagina_origen, descripcion]
      );

    if (numero_pagina_destino)
      await conn.query(
        "UPDATE opcion SET numero_pagina_destino= $3 WHERE id_aventura = $1 AND numero_pagina_origen = $2",
        [id_aventura, numero_pagina_origen, numero_pagina_destino]
      );
