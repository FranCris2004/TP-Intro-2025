...
document
  .querySelector("#form-registro form")
  .addEventListener("submit", async (e) => {
    ...
    const confirmar = document
      .getElementById("validar-contraseña")
      .value.trim();
...
document
  ...
  .addEventListener("submit", async (e) => {
    ...
    const contrasenia = document
      .getElementById("contraseña-login")
      .value.trim();
    ...
    try {
      ...
      const respuesta_text = await respuesta.text();
      const respuesta_json = JSON.parse(respuesta_text);

      if (!respuesta.ok) {
        console.error("Respuesta no-OK (no es JSON):", respuesta_text);

        throw new Error(
          JSON.stringify(respuesta_json) || "Credenciales incorrectas"
        );
      }

      const usuario = respuesta_json;
